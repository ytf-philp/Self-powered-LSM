...
def process_dataset(batch, tokenizer):
    ...
    suffix_input_ids = tokenizer("<|eot_id|><|start_header_id|>assistant<|end_header_id|>").input_ids[1:] # remove bos token
    ...
    suffix_input_ids += label_ids
    suffix_attention_mask += [1] * len(label_ids)
    #label_attention_mask = [1] * len(label_ids)
...
def load_speech_text_paired_dataset(
    ...
    dataset = datasets.load_dataset('json', data_files="self-powered/new_aug_llama3/final_shuffle.jsonl", split='train')
    dataset = dataset.map(
        process_dataset,
        fn_kwargs={
            "tokenizer": tokenizer,
...
def offline_process(
    ...
    manifest_files="llama3_stage1_sft",
    lm_path="//self-powered/Meta-Llama-3-8B-Instruct",
    num_proc=48,
):

...
Text_Format = (
    ...
    "USER: Answer the questions in speech based on the CONTEXT given,your answer is only true or false, you don’t need to answer anything else \n"
    "CONTEXT: {references}\n"
    "SPEECH: "
)
...
def process_dataset(batch, tokenizer, instruction):
    ...
    # instruction prompt
    input = Text_Format.format(references=batch["passage"])
    ...
    ### response
    output = batch["answer"] + tokenizer.eos_token
...
def load_speech_text_paired_dataset(
    ...
    dataset = datasets.load_dataset('json', data_files="/self-powered/evaluate/QA/data/Bool_QA/boolQA_samples_audio.json",split="train")
    dataset = dataset.map(
        process_dataset,
        fn_kwargs={
            "tokenizer": tokenizer,
...
def offline_process(
    dataroot="/self-powered/evaluate/QA/data/process",
    manifest_files="evaluate_boolqa_test",
